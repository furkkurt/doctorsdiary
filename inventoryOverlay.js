class inventoryOverlay extends Phaser.Scene{
  ...
  create(data){
    this.inventory = this.add.sprite(-20,this.cameras.main.centerY,"inventory").setOrigin(0, 0.5).setScale(6).setScrollFactor(0).setDepth(500)
    this.from = data.from
    ...
    this.item1 = this.add.sprite(this.inventory.x+20, this.inventory.y-140, "").setOrigin(0).setScale(1.5).setVisible(false).setScale(.33).setDepth(499)
    this.item2 = this.add.sprite(this.inventory.x+20, this.inventory.y-40, "").setOrigin(0).setScale(1.5).setVisible(false).setScale(.33).setDepth(499)
    this.item3 = this.add.sprite(this.inventory.x+20, this.inventory.y+60, "").setOrigin(0).setScale(1.5).setVisible(false).setScale(.33).setDepth(499)
  ...
  pick(item, collect, msg, dialogue) {
    ...
    if(msg != ""){
      dialogue.dialogue(msg, "docPort", null, "1", null, "Doctor")
  ...
  updateSlots() {
    // Only update if we have the slot and items initialized
    if (!this.slot || !this.item1 || !this.item2 || !this.item3) {
      return;
    }
  ...
  lamp() {/** 
    ...
    }*/ console.log("gerek yok amk")
  }

  kidsRoom() {
    // If progress is 3, trigger nurse dialogue in scene3 instead of starting scene4
    if (progress == 3) {
