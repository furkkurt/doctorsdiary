class scene1 extends baseScene {
  ...
  create(data){
    ...
    this.scene.bringToTop("brightnessOverlay") //bu üstte kalsı:w
    ...
    // Check for progress 8 cutscene
    if (progress == 8) {
      this.startProgress8Cutscene();
      return; // Exit early to prevent normal scene setup
    }
...
lockControlsFor(ms) {
  ...
  startProgress8MedsCutscene() {
    //ilaçları alıyoruz ve progress oldu 9
    progress = 9;
    if (this.isTransitioning) return;
    this.isTransitioning = true;
  ...
  startMedsCutscene(){
    //nurse gördük xanaxı attı salvo uykuya daldı salvo progress etti 6
    progress = 6
    if (this.isTransitioning) return;
    this.isTransitioning = true;
    // Fade to black
